...
fn generate_custom_model(fields: &Fields, model: &CustomModel) -> proc_macro2::TokenStream {
    ...
    } in fields
    {
        ...
        * leading_colon: Option<Token![::]>
           This field represents an optional leading double colon (::) in the path.
           For example, in the path ::std::collections::HashMap, the leading colon is present.
           segments: Punctuated<PathSegment, Token![::]>
           This field represents the segments of the path, separated by double colons (::).
           Each segment is a PathSegment, which typically consists of an identifier and optional generic arguments.
           The Punctuated type is a utility provided by syn to handle sequences of elements separated by punctuation.
        */
    ...
    let struct_ident: Ident = match Ident::from_string(name) {
        Ok(ident) => ident,
